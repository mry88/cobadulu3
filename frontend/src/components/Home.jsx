...
function NextArrow(props) {
  ...
  return (
    <button onClick={onClick} type="button" className="z-0 absolute top-0 right-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-next>
      <span class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 hover:bg-gradient-to-r from-green-400 via-green-500 to-green-600 group-focus:ring-1 border border-black group-focus:ring-black dark:group-focus:ring-gray-800/70 group-focus:outline-none">
      ...
      <div className={`flex-grow overflow-y-auto`}>
        ...
        <div className="ml-20 mr-20 ">
                ...
                filteredProducts.map((product) => (
                  <div key={product._id} className="flex flex-col bg-gray-100 shadow-lg border-2 border-gray-900 rounded-lg">
                    ...
                    <div className="flex flex-col grow p-4">
                      <h3 className="text-dark font-bold text-2xl pb-2">{product.name}</h3>
                      <p className="text-dark text-sm font-medium">{product.category.name}</p>
                      <p className="text-dark text-xl font-bold mt-1">Rp. {product.price}</p>
                    </div>
          ...
          <button
            ref={scrollButtonRef}
            onClick={scrollToTop}
            className="fixed bottom-10 right-10 bg-gray-100 text-gray-200 px-4 py-4 rounded-full border border-black focus:outline-none hover:bg-gradient-to-r from-green-400 via-green-500 to-green-600 focus:ring-1 focus:ring-black"
          >
            <svg class="w-[18px] h-[18px] text-black dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 8">
